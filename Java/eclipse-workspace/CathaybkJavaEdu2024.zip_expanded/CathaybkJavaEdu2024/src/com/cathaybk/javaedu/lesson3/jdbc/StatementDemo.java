...
public class StatementDemo {
    ...
    public static void main(String[] args) {
        ...
        //取得連線、Statement物件
        try (Connection conn = DriverManager.getConnection(connUrl, "student", "student123456");//取得連線
            ...
            //ResultSet物件儲存查詢結果
            ResultSet rs = stmt.executeQuery(sqlStr);//執行sql
            ...
            while (rs.next()) {
                sb.append("查詢結果：{ ID： ").append(rs.getString(1)).append("   PWD：").append(rs.getString(2)).append("   NAME：")
                        .append(rs.getString("NAME")).append(" }");//append是指直接將值加入同一格
